...
function build (store) {
  ...
  */
  ...
  function position (opts) {
    if (!opts.term || !opts.appId) {
      return Promise.reject(Error('an array of seed term and appId is required for this strategy'));
    }

    let position = 0;
console.log(store.search(opts));
    store.search(opts).forEach(item => {
      position++;

      if(item.appId == opts.appId) {
        return null;
      }
    });
  ...
  strategies[c.POSITION] = position;
